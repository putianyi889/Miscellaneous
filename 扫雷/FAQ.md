...
- 我初级xx秒，中级xx秒，高级xx秒，水平怎么样？
  ...
  - 我们一般认为手速、移动、反应、算力是扫雷所需的天赋。一项突出可以成为大雷圣，两项突出可以成为小雷帝，三项突出可以成为大雷帝，四项突出只有鞠帝一人。一般人能sub60已属不易。
...
- 鼠标设置有何讲究？
  ...
  - 要根据个人情况仔细调节鼠标和鼠标垫之间的摩擦力。影响摩擦力的因素有：鼠标垫脏了；鼠标垫湿了（往往由湿手、汗手造成）；鼠标垫材质；鼠标脚贴材质；鼠标重量；上身姿势。
  - 上身姿势是很容易被忽略的一点。上身姿势会影响手对鼠标的压力，进而影响摩擦力。举例来说，如果右手握鼠标，那么上身右倾时摩擦力会明显比上身左倾时摩擦力大。用非惯用手支撑上身部分重量也是一种减少摩擦力的办法。
...
- 开局有什么技巧吗？
  - 开局一般分为两种：角开和画圈
  - 角开是指从一角开始连击。角开套路固定，操作简单，胜率更高，建议还处在学习进步阶段的选手使用。
  - 画圈是指开局随机大片乱点。画圈更消耗体力；操作难度大，因为画圈时会丢失操作精度，画圈以后第一时间的定位比较困难；开局更难开片，但是成功开片之后的收益更高。画圈适合技术较成熟，打算刷纪录的选手使用，尤其是初中级。初中级往往可以遇到一两个op的图，如果开局可以把op都点出来那么这局有相当概率直接破纪录。
  - 顶尖高手也少有高级画圈的，是因为高级过程很长（导致op较多），雷密度大（导致op较小），画圈开局的风险高于回报。操作非常强的选手会采用折中方案，即角附近小范围画圈（如鞠泽恩）或顶行快速横扫（如卡米尔）。
...
- RQP、QG、STNB是什么？
  - 竞技扫雷诞生之初评定水平的指标只有一个Time（RTime），time越小实力越强。有人发现不同局面难度差别其实很大，于是开发出了评价局面难度的3BV（bv）指标，并用3BV/s（bvs）评价水平，bvs越高实力越强。又发现time和bvs一般不能两全，为了评价在高bvs同时获得低time的能力，用time/bvs得到了RQP（实际上是(time+1)/bvs，不过这不重要），rqp越小实力越强。time、bvs、rqp是国际扫雷圈认可的三项指标。
...
- 梁山分、定阶、软实力？
  - 我们常说的梁山分（LSF）和定阶指的是以国内顶尖选手（National Top，NT）为锚点对一个玩家实力的估计。玩家的time纪录高度依赖运气，如何根据一个玩家当前扫出来的结果推算它的实力对应的time纪录？这就是梁山分和定阶的背景。梁山分基本上就是软实力。考虑到nf的bvs和stnb天生弱势，计算梁山分时有一个nf加成：LSF=0.94\*软实力+0.06\*nf软实力
  - 软实力的计算采用的是三级别time、bvs、stnb前五，以削弱极端运气带来的影响。为了进一步削弱初级的运气带来的影响，初级最小bv定为10。具体算法并不需要了解，只需要知道如果你的一个录像是自己的time、bvs、stnb之一的前五（或nf前五）
  - 定阶直接通过软实力得到，分为定阶和nf定阶。所以nfer在相同定阶下梁山分会略高，fler也可以通过传nf录像来涨分，尽管定阶大概率不会有任何变化。
  
- PB、ASB、典型时、战力、奖牌、啄？
  - PB（Personal Best）表示玩家在某个bv发挥出的最好成绩。
  - ASB（All Sumed Bvs）是所有pb的bvs之和。
  - 典型时是所有pb的time之和。未收集的bv，按照初级10.00，中级60.00，高级240.00计。
